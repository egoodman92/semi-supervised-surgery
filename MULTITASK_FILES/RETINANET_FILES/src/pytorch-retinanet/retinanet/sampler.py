...
from .sampler_utils import setup_logger

from .dataloader import CocoDataset, CSVDataset, collater
#, Resizer, AspectRatioBasedSampler, Augmenter, Normalizer, BalancedSampler 

import random 


def aggregate_indices_per_class(csv_dataset): 
    groups_by_label = {}
    for label in csv_dataset.labels:
    ...
    #print(groups_by_label) 
    print("Using new Balanced Sampler") 
    #print(indices) 

    for index in indices:
        annotations = csv_dataset.load_annotations(index)

        if annotations.shape[0] == 0:
            groups_by_label[-1].append(index)
            continue

        #print("annotations: ", annotations[:, 4]) 

        best_label = -1
        group_size = float('inf')
        for label in groups_by_label:

            # Second part enforces that the "best label" is the one with the smallest # of frames currently 
