...
def augment_raw_frames(raw_frames, method='val', img_size=224):
    if method == 'ten':  # 4 corners + center crop. 50% flipped
        ...
        #changed the Sequential augmentation method by removing the crop to fixed size - Emmett
    ...
    pad_h = 32 - h % 32
    pad_w = 32 - w % 32
    results = np.zeros((num_frames, h+pad_h, w+pad_w, c), raw_frames[0].dtype)
    for i in range(num_frames):
        results[i, :h, :w, :] = augDet.augment_image(raw_frames[i])
        #print("SHAPES", raw_frames[i].shape, results[i].shape)
    return results
