# PoseWarper Surgery Hands Experimentation
Code to experiment with PoseWarper framework and the Youtube Surgery Hands Dataset. 


## Data
To download Youtube Surgery Hands data to work with the framework, follow the instructions below.
...
To download pretrained models to load into the PoseWarper framework, follow the following instructions.

Either download the models folder from [Google Drive](https://drive.google.com/file/d/1Tk8dXtLPKxnIbf8FmCm9gqrzXtmpVNDa/view?usp=sharing) and unzip it in the `PoseWarper` directory, or use the following to download the weights from SAIL.
