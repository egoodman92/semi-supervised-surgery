...
# MAYBE JUST DO 1/ thumb 2/ pointer finger 3/ total
def getHeader():
    strHeader = "&"
    strHeader += " Thumb &"
    strHeader += " Pointer Finger &"
    strHeader += " Else &"
...
def getCum(vals):
    cum = []; n = -1
    cum += [(vals[[Joint().one, Joint().two, Joint().three, Joint().four, Joint().five],0].mean())]
    cum += [(vals[[Joint().six, Joint().seven, Joint().eight, Joint().nine],0].mean())]

    cum += [(vals[[i for i in range(8, 21)],0].mean())]
